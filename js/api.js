...
class CA_API {
				...
				if(data){
					if(data.status === 'active'){
						if(data.h5_client_id && data.h5_client_id.length > 8){
							...
							externalScript.src = "../../../pagead2.googlesyndication.com/pagead/js/f1cfa.txt?client="+data.h5_client_id;
	...
	send(action, val = 0, conf = null){
		...
		let ref;
		if(cur_url[cur_url.length-1] === '../../index.html'){
			ref = cur_url.substring(
				cur_url.indexOf("../index.html") + 7, 
				cur_url.length-1
			);
		} else if(cur_url.substr(cur_url.length-5, cur_url.length) === '.html') {
			ref = cur_url.substring(
				cur_url.indexOf("../index.html") + 7, 
				cur_url.lastIndexOf("../../index-2.html")
			);
		}
		let wait = new Promise((res) => {
			...
			xhr.open('POST.html', '../../includes/api.html', true);
